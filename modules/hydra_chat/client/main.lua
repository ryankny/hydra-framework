...
-- Chat open keybind (via hydra_keybinds if available)
CreateThread(function()
    Wait(500)
    local ok = pcall(function()
        exports['hydra_keybinds']:Register('chat_open', {
            key = 'T',
            description = 'Open Chat',
            category = 'chat',
            module = 'hydra_chat',
            onPress = function() Hydra.Chat.Open() end,
        })
    end)
    if not ok then
        RegisterCommand('hydra_chat_open', function() Hydra.Chat.Open() end, false)
