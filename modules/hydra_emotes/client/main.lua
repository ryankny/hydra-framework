...
-- Cancel keybind (via hydra_keybinds if available)
CreateThread(function()
    Wait(500)
    local ok = pcall(function()
        exports['hydra_keybinds']:Register('emote_cancel', {
            key = cfg.cancel_key,
            description = cfg.cancel_key_description,
            category = 'emote',
            module = 'hydra_emotes',
            onPress = function() Hydra.Emotes.Cancel() end,
        })
    end)
    if not ok then
        RegisterCommand('+hydra_emote_cancel', function() Hydra.Emotes.Cancel() end, false)
